...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-auth-backend-pam runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-auth-backend-pam.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ auth_backend_pam")
...
def lint(c):
    ...
    """
    c.run("flake8 django-auth-backend-pam tests")
...
def release(c, bumpsize=""):
    ...
    import django_auth_backend_pam
    ...
    c.run(
        "git tag -a {version} -m 'New version: {version}'".format(
            version=django_auth_backend_pam.__version__
        )
    )
    c.run("git push --tags")
